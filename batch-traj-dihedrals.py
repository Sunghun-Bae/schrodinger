...
dihedral_def = {
    # standard nucleic acids dihedral angles
...
def get_dihedral_atom_ids(st, chain, resSeq, angle_name):
    ...
    for a in st.atom:
        if (a.property['s_m_chain_name'] == chain) and \
            (a.property['i_m_residue_number'] in residue_numbers) :
            atoms_.append(a)

    atom_ids=[]
    for (offset, atom_name) in dihedral_def[angle_name]:
        for a in atoms_:
            if (a.property['i_m_residue_number'] == (resSeq+offset)) and \
    ...
    for input_cms in args.cms:
        ...
        for c in cms_model.chain:
            if c.name == args.chain:
                sequence = "".join([ r.pdbres.strip()[0] for r in c.residue ])
        resSeq_start = 1
        resSeq_stop  = len(sequence)
        
        list_of_dihedrals = [] # dihedrals to analyze
        info_of_dihedrals = []
        for angle_name in names:
            for resSeq in range(resSeq_start, resSeq_stop+1):
                ...
                list_of_dihedrals.append(obj)
                info_of_dihedrals.append((angle_name, resSeq))
                #print()
                
        list_of_results = analysis.analyze(trj[startframe:endframe], *list_of_dihedrals) 
        #print("list_of_results=", list_of_results[:20])
        # analysis.analyze returns a list of list

        for (angle_name, resSeq), results in zip(info_of_dihedrals, list_of_results):
